define([ 'underscore', 'backbone' ], function(_, Backbone) {
	...
	memo.save(
			...
			{
				success : function() {
					console.log("After create memoList: "
							+ JSON.stringify(memoList));
					console.log("After create memoList.length: "
							+ JSON.stringify(memoList.length));
				}
			}).then(
			function() {
				console.log("fetch");
				return memoList.fetch({
					success : function() {
						console.log("After fetch memoList: "
								+ JSON.stringify(memoList));
						console.log("After fetch memoList.length: "
								+ JSON.stringify(memoList.length));
					}
				});
			}).then(
			function() {
				...
				return tempMemo.save({
					content : "Kenichiro"
				}, {
					success : function() {
						console.log("After save memoList: "
								+ JSON.stringify(memoList));
						console.log("After save memoList.length: "
								+ JSON.stringify(memoList.length));
					}
				});
			...
			function() {
				...
				return tempMemo.destroy({
					success : function() {
						console.log("After destroy memoList:"
								+ JSON.stringify(memoList));
						console.log("After destory memoList.length:"
								+ JSON.stringify(memoList));
					}
				});
